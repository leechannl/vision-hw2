...
image structure_matrix(image im, float sigma) {
  ...
  image S_ = make_image(im.w, im.h, 3);
  for (int i = 0; i < im.w; i++) {
    for (int j = 0; j < im.h; j++) {
      set_pixel(S_, i, j, 0, get_pixel(Ix, i, j, 0) * get_pixel(Ix, i, j, 0));
      set_pixel(S_, i, j, 1, get_pixel(Iy, i, j, 0) * get_pixel(Iy, i, j, 0));
      set_pixel(S_, i, j, 2, get_pixel(Ix, i, j, 0) * get_pixel(Iy, i, j, 0));
    }
  }
  S = convolve_image(S_, make_gaussian_filter(sigma), 1);
  return S;
}
